...
if (!empty($post) && !empty($post->accio) && $post->accio == 'update') {
    ...
    if ($session != null) {
        try {
            $resultat = LlistaUsuaris::updatePersona($post->persona);
            if ($resultat) {
                $response = array(
                    "success" => true,
                    "data" => null,
                    "message" => "Persona actualitzada correctament.",
                );
            } else {
                $response = array(
                    "success" => false,
                    "data" => null,
                    "message" => "Hi ha hagut un problema al actualitzar la persona.",
                );
            }
        } catch (\Throwable $th) {
            $response = array(
                "success" => false,
                "data" => null,
                "message" => "Error al actualitzar la persona."
            );
        }
    } else {
        $response = array(
