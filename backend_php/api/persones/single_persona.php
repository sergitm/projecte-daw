...
if (!empty($post) && !empty($post->accio) && $post->accio == 'getPersona') {
    $session = Session::Get(['session_id' => $post->session_id]);
    if ($session != null) {
        try {
            $persona = LlistaUsuaris::getSingle($post->persona_id);
            if ($persona) {
                $response = array(
                    "success" => true,
                    "data" => $persona,
                    "message" => "Persona retornada.",
                );
            } else {
                $response = array(
                    "success" => false,
                    "data" => null,
                    "message" => "No s'ha trobat cap persona.",
                );
            }
        } catch (\Throwable $th) {
            $response = array(
